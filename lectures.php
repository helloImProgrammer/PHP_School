				...
				<p>Dersleriniz ve Sınav Şekliniz aşağıdaki gibidir</p>
							...
							if(!empty($startdate) && !empty($finish_date)){
								if(strtotime($startdate)<=strtotime(date("Y-m-d")) &&   strtotime(date("Y-m-d")) <= strtotime($finish_date) && $examActive == 1 ){
										...
										WHERE exams.Eid is null and lectures.TSid='.$TSid);  
										...
										<option value="<?php echo $row[0] ?>" ><?PHP echo $row['Lcode']." - ".$row['Lname']." - ".$row['Pname']  ; ?></option>
										  <?PHP } }
												...
												$message='Sınav şekli girilecek ders bulunamadı';
											}
											...
											$sorgu = "INSERT INTO exams(Eid, TSid, Lid,Pid, ETypeid) VALUES (null,$TSid,$Lid,$Pid,$Etypeid)";
																	//exam tablosunun insert sorgusu
											$sonuc = mysqli_query($con,$sorgu);
											
											

											if ($sonuc){	//insert başarılı ise alttaki mesaj basılır
												$sign_up = 'Kayıt işleminiz başarılı';
												header("Refresh:1; url='lectures.php'");
										...
										?>
															...
															<p><input type="submit" name="Kaydet" value="Kaydet"/><br/>
															</form>
													<?php   }else{
